"""Personas router for person management endpoints.

This module provides API endpoints for:
- Creating persons
- Listing persons with pagination
- Retrieving person details (with country information)
...
router = APIRouter(prefix="/personas", tags=["Persons"])
...
def get_persona_repository(session: Session = Depends(get_session)) -> PersonaRepository:
    ...
    Returns:
        PersonaRepository: Repository instance
    """
...
def get_pais_repository(session: Session = Depends(get_session)) -> PaisRepository:
    ...
    Returns:
        PaisRepository: Repository instance
    """
...
def create_persona(
    ...
    Args:
        ...
        pais_repo: Country repository
    
    Returns:
        PersonaResponse: Created person
    
    Raises:
        HTTPException: If creation fails or country not found
    """
    try:
        ...
        if persona.pais_id is not None:
            ...
            if not db_pais:
                raise HTTPException(
                    ...
                    detail=f"Country with id {persona.pais_id} not found"
                )
